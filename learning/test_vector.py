...
class TestVector(TestCase):
  ...
  def test_create(self):
    # 성공 테스트 더미
    success_dummy = [
      (Vector, range(1), Vector(range(1))),# 1-1. 기본 생성자로 객체를 생성하는 경우 -> 예외 발생
      (Vector, range(10), Vector(range(10))),  # 1-2. 시퀀스 형을 매개변수로 생성자 호출하는 경우 -> 성공
      (Vector, range(10), Vector(range(10))) # 1-3. 문자열 매개변수를 전달하여 생성자를 호출하는 경우 -> 예외 발생
    ]
